...
#Read the Email File
a=read.csv("D:/spl/Project/InputData/email",header = TRUE,check.names=T,stringsAsFactors = FALSE)
#Selecting  emails
a10=a[1:1011,1:15]
a111=data.table(a10)
a11=as.data.frame(a111[ , list( To = unlist( strsplit(To,"\n") ) ) , by = list(ï..Mailbox, Date, From.Name, From.Address, ReplyTo, Cc, Bcc, Subject, Priority, Flags, Message.Id, Message, Attachments, Folder) ])
##View(a11)
#selecting message column from the emails
messagecol=a11[,12]
#Read dictionary
SentimentDict<-read.delim(file='D:/spl/Project/InputData/Dictionary/AFINN/AFINN-111.txt', header=FALSE, stringsAsFactors=FALSE)
